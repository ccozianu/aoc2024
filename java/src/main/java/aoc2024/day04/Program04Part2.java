...
public class Program04Part2 {
	
	static final char[] MAS= {'M','A','S' };
	
	static int checkDirection(char[][]arr, int x, int y) {
		return ( MAS[1] == arr[x] [y] 
						&& ((MAS[0] == arr[x-1][y-1] && MAS[2]==arr[x+1][y+1])
							|| (MAS[2] == arr[x-1][y-1] && MAS[0]==arr[x+1][y+1]))
						&& ((MAS[0] == arr[x-1][y+1] && MAS[2]==arr[x+1][y-1])
								|| (MAS[2] == arr[x-1][y+1] && MAS[0]==arr[x+1][y-1])) ) 
				?1:0;
	}
	...
	private static char[][] readInput() {
		try {
			try(BufferedReader reader = new BufferedReader(new InputStreamReader(Program04Part2.class.getResourceAsStream("/input04.txt")))) {
				String currLine;
