...
public class Program13Part2 {
	...
	private static class LongEq2x2 {
		...
		public long[] solveIt() {
			...
			if (m10_reduced==0) {
				// if the input requires us to enumerate diophantine solutions
				// we'll do it then :)
	...
	public static void main(String[] args) {
		try {
			...
			var input = readInput();
			Program13Part2 program = new Program13Part2(input);
	...
	private static LongEq2x2[] readInput() {
		try {
			...
			try(BufferedReader reader = new BufferedReader(new InputStreamReader(Program13Part2.class.getResourceAsStream("/input13.txt")))) {
				String currLine;
				while (true) {
					...
					long[] rowX = {Long.parseLong(matcherX.group(1))+10000000000000l, Long.parseLong(matcherX.group(2))+10000000000000l};
